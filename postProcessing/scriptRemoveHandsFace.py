...
inputBvh = "/Users/abdu/Desktop/EHU/TFG/dataset/bvh/allBvhs"
outputBvh = "/Users/abdu/Desktop/EHU/TFG/dataset/noFaceNoHands"

# train partition
if not os.path.exists(outputBvh): os.makedirs(outputBvh)
i=0
for filename in sorted(os.listdir(inputBvh)):
    bvhFilename = os.path.join(inputBvh, filename)
    exportFilename = os.path.join(outputBvh, filename)
    bpy.ops.import_anim.bvh(filepath=bvhFilename, axis_forward='Y', axis_up='Z')
    #IMPORT_ANIM_OT_bvh.
    ...
    if ob.type == 'ARMATURE':
        ...
        for bone in armature.edit_bones:
            if bone.name in bonesToRemoveFace or bone.name in bonesToRemoveHands: 
